# Week 7. Policy Analysis, Nonprofit Sector

## C5. Agenda setting
- Goals
  - Discuss how agenda setting works and how issues are framed
  - Discuss the major policy actors and their roles in setting the policy agenda
  - Use policy theories to explain agenda setting

### How does agenda setting work?
- policy agenda is a series of streams, circumstances, and activities
- **problem stream**. potential policy problems
- **policy stream**. potential solutions to problems
- **politics stream**. issues and solutions that emerge as policy outcomes
- a **focusing event** opens a **policy window** for policies to be made
  - e.g. 9/11
  - e.g. collapse of Minnesota bridge
  - e.g. Sandy Hook

### The president as agenda setter
- 1800s, presidents were **literalists**, only exercising powers in the Constitution
- 1900s saw extension of **presidential stewardship**
  - bully pulpit
  - State of the Union address

### Congress and the policy agenda
- when both congress and president are of the same political party, agenda is often reframed

### Congressional-executive relations and agenda setting
- Presidents advance agenda items (they focus on agenda items) that are likely to become policy

### The courts and the policy agenda
- before: **constitutionalism**
- now: **judicial positivism**. using legal reasoning to determine the substantive meaning and expected outcomes of policy
- examples of courts shaping agenda
  - Brown v. Board of Education
  - Obergefell et al. v. Hodges

### Pressure groups and agenda setting
- **think tanks**. pressure groups devoted almost solely to policy research
- **Common Cause** for car safety was one of the first noteworthy **public interest groups**

### Bureaucracy and agenda setting
- policy often needs finetuning, which is done by bureaucracy
- bureaucracy can shape policy agenda
  - e.g. EPA 2015 Clean Power Plan

### Media and agenda setting
- media can focus attention on certain problems
- social media is becoming more important in setting agenda

### Elections and the policy agenda
- **realignment** is long-lasting majority for a political party, giving tremendous ability to set agenda
- current: **dealignment**, divided government

### Cultural change and agenda setting
- as culture changes, what is deemed a risk changes, and thus policy changes as well

### Biases in political participation and agenda setting
- **target populations**
  - some groups are labelled "undeserving"

### Explaining agenda setting

#### Multiple streams theory
- the agenda-setting process is **reactive**

#### Advocacy coalition framework (ACF)
- the agenda-setting process is deliberate
- explains the agenda during stable times

## C6. Policy formation
- Goals
  - Discuss how policy is formulated
  - Identify the major participants in policy formation
  - Use policy theories to explain policy formation

### How is policy formed?
- **policy formation**. the process of using normative and empirical methods to define a policy goal, explore alternative solutions, and choose a preferred solution
- policy problems are often linked to **market failures**
- **demonstration projects** help guide large-scale policy planning efforts
- policy planning must resist **goal drift/shift**

### Major actors in policy formation
- congress, president, judiciary
- Congressional Budget Office
  - estimates dollar cost of policy implementation
- Congressional Research Service
  - policy research for Congress and congressional committees
- Government Accountability Office
  - focuses on policy evaluation and auditing

#### Presidential policy formation
- **Bureau of the Budget (BOB)**
  - president can propose a national budget annually
- **Executive Office of the President (EOP)**
  - administrative support for the president

#### Courts and policy formation
- negation of policies that don't promote fair outcomes guides policymaker's future proposals

#### Interest groups and policy formation
- may conduct policy formation through executive (president; via task forces) or courts (lawsuits)
  - can also file _amicus curiae_ briefs

#### Public bureaucracies and policy formation
- bureaucracies are run by people with political interests
- changing who is in charge of bureaucracies can change policy

#### International influences on policy formation
- globalism, "free market" capitalism often hurts small countries who are subject to international competition

#### Citizen stakeholders and policy formation
- citizen formed policy: **bottom-up policymaking**, **grassroots movement**
- important venues for citizen participation
  - **citizen advisory boards**
  - **collaborative policymaking**

#### Policy entrepreneurs, change drivers, ideological drivers and policy formation
- policy entrepreneurs
  - Koch brothers
  - George Soros
- change drivers seek to shape the formation process through network building
- ideological drivers focus on narrowly defined issues

### Demographic changes and policy formation
- examples of demographic shifts
  - divorced families
  - children born to unwed parents
  - immigrants
  - "graying" of America

### Explaining policy formation

#### Advocacy Coalition Framework
- how do stakeholders converge into coalitions to advance policy?
- describes policy formation in **subsystems** (substantive policy area/territorially defined policy system)

#### Policy Diffusion model
- how do public policy ideas spread globally?
- provides a more macro-level understanding

#### Institutional Analysis and Design Framework
- action arena: defined rules and allowable actions
- offers a logical basis of making policy related to **common pool resources**

## C7. Policy implementation (PI)

- Chapter goals
  - Discuss four major types of PI
  - Discuss the major participants in PI
  - Use policy theories to explain policy formulation

### A theoretical model of policy implementation
- Richard Matland (1995): six factors shaping PI success
  1. **statutory compliance**
      - **statute**. a written law passed by a legislative body
      - do implementors comply with the policy?
  2. **bureaucratic accountability**
      - bureaucrats should be held accountable to implement policy in a non-biased manner
  3. **statuatory goals accomplished**
      - a desired output was produced
  4. **local goals accomplished**
      - do constituents benefit from the PI?
  5. **political climate improvement**
      - successful PI should make people more partial to public policy
  6. **learning**
      - policies improve over time
- PI studies: **top-down** or **bottom-up** approaches

#### Top-down
- Assumptions
  - Statutory language is complete, concise, and applicable
  - Implementation is an administrative function
- **barrier**. anything that impedes compliance

#### Bottom-up
- Assumptions
  - Policy is implemented by local-level administrators and elected officials
  - Statutes are not complete and concise

- Top-down and bottom-up are brought together with **ambiguity-conflict model**
- dimensions in PI
  - **policy ambiguity**
    - **ambiguity of goals**
      - top-down wants to reduce ambiguity, while bottom-up uses it to negotiate
    - **ambiguity of means**
      - technology
      - money
  - **policy conflict**
    - disagreements over policy
    - disagreements over implementation

#### Adminstrative implementation (low ambiguity, low conflict)
- goals will be met as long as resources are provided

#### Political implementation (low ambiguity, high conflict)
- power determines resource allocation which determines outcomes

#### Experimental implementation (high ambiguity, low conflict)
- outcomes depend on which actors are most active
- possible for implementation to be **captured**. implementation is shaped by individual and group interests at the local level

#### Symbolic implementation (high ambiguity, high conflict)
- symbolic policies are intended to reafirm values and associated policy goals

### Issues surrounding implementation

#### Steerage
- **steerage**. keeps policy focused on intended goals
- exists at two levels:
  - **adminstrative**
  - **political**: oversight

#### Tractability
- **tractability**. the ability of public policy to control, influence, or reshape phenomenon in order to achieve a policy goal
  - need to know **why** the problem is occurring

#### Target group identification
- **target groups**. individuals or organizations whose behavior is directly impacted by public policy

#### Federalism
- **unfunded mandates**
- **funded mandates**; provided with **grants-in-aid**

#### Veto points
- **veto points**. places within the policy process where policy can be changed or eliminated
  - **organizational culture** is a possible veto point
  - **legal standing** is another veto point

#### A changing policy environment
- possible changes in 
  - target group
  - technology
  - public attention
  - public opinion

#### Slippage
- **slippage**. a reduced ability to produce outputs consistent with policy goals

### Major participants in policy implementation

#### Bureaucracy
- **adminstrative law** written into the **Federal Register**
- administrative laws guide the day-to-day implementation of statutes

#### Pressure groups
- involved in
  - public hearings
  - information campaigns
  - lobbying
  - expert testimony
- leads to the **iron triangle**: pressure groups, administrative agencies, and legislative committees

#### Clientele
- social values determine which **clientele** is seen as deserving

#### Judicial system
- where people need to demonstrate **legal standing**
- setting out of courts: **alternative dispute resolution (ADR)** 

### Explaining policy implementation

#### Institutional Analysis and Development Framework (IAD)
- IAD focuses on marketable public goods
- IAD encourages cooperative resource use and management

#### Advocacy Coalition Framework (ACF)
- focuses on iron triangle
- offers some explanation of how policy moves from high ambiguity to low ambiguity via learning

#### Constructivist Approach
- constructivism believes clientele or policy target groups are socially constructed
- useful for explaining inequality

--- 

## Lecture 13. Hometown analysis, Nonprofits

### Hometown analysis

#### Health data
- health effects from indoor vs. outdoor pollution: particulate matter

### 501(c)(3) Nonprofits
- Tax deductible
- smaller nonprofits
  - operating budget (OB): $500k
- private foundations
  - WK Kellogg Foundation

#### Revenue sources for reporting public charities
- Fees for services and goods from private sources
- Fees for services and goods from government sources
  - **33% must come from the public**
  - e.g.
    - donations under 2% of nonprofit's revenue
- Private contributions
- Government grants
- Investment income

### Contracts
- What does government fund, and what does it contract out to private sector/nonprofits?

---

## Lecture 14. Nonprofits and Form 990

### Form 990
- Use the Attorney General's website to find 990s
- Get the **Employer identification number (EIN)**

### Structure of nonprofit
- Board of Directors
  - governs over the C-suite

### Expenses
- Audit/Review: 7K to 20K
  - every tax filing needs an audit (>500K) or review (>250K)
- Salaries